...
### Show it, do not only describe it

**A feature with a visible surface gets a screenshot.** Prose explains why
something exists; a picture is what makes a reader recognise it when they see
it. A handbook page that describes a dialog nobody has ever seen is doing half
its job, and the README and the website both pull from this well.

Capture one when the feature adds any of:

| Add | Because |
|-----|---------|
| A modal or a new panel | It is a surface a reader has to recognise |
| A settings section | "Where do I turn this on" is the first question |
| A distinct state of an existing view | A badge, a marker, a banner — the thing that would otherwise be invisible |
| A gesture or an animation | A still cannot show a drag; a clip can |

Skip it for a menu entry that changes nothing visually, a config-only change, or
a surface too close to one already captured. **A handful of good shots beats one
per page** — an interface photographed exhaustively reads as a catalogue.

Never hand-grab a window. Screenshots are generated by driving the **real built
app** with Playwright against the deterministic playground repos, so they cannot
go stale silently:

```bash
npm run screenshots my-feature   # one shot, by name
npm run screenshots:clips        # stills plus animated WebP clips
```

Add an entry to `examples/screenshots/shots.config.mjs` — `out` (the file name),
`repos` (which playground repo, so the state is reproducible), optional
`prepare` to put the repo into a state and `drive` to put the UI into one. The
full recipe, including the `--shot` store bridge, is in
`examples/screenshots/README.md`.

Everything is **WebP** — stills and clips alike. Needs `ffmpeg`. A motion clip
(`clip-*.webp`) is the right answer for a gesture and the wrong one for a
dialog, which a still shows better and in a tenth of the bytes.

Adding a new playground scenario for a shot is normal and cheap: a screenshot
that needs a repo in an odd state is a scenario, not a `prepare` hack.

**Never photograph the machine.** A feature that reads outside the repository —
`~/.ssh`, a global git config, a credential store — must be pointed at a
generated fake for its shot, not captured as it is on your disk. The SSH shot is
the worked example: it generates a throwaway keypair and redirects the app with
`GITCITO_SSH_DIR`, which `main/ssh.ts` honours **only** under `--shot`. Masking
values with `***` is the wrong fix — it makes the UI look broken and puts
redaction logic in production code.
...
- [ ] A **screenshot** (or a clip, for a gesture) captured via
      `npm run screenshots`, if the change added a modal, a panel, a settings
      section or a new visible state — and referenced from its handbook page
