...
| Перемикач | Що робить |
|---|---|
| **↑ / ↓** | Перейти до попередньої або наступної зміни |
| **Увесь файл** (лише поруч) | Увесь файл, кожна зміна на своєму місці; вимкнено — лише змінені фрагменти |
...
**Увесь файл.** Вигляд поруч показує весь файл — старий ліворуч, новий праворуч — і відкривається на першій зміні: червоні й зелені смуги позначають змінені рядки, штрихування заповнює бік, де рядка немає, напрямні відступів позначають кожен рівень, а лінійка праворуч від диффу показує весь файл — червона доріжка для видалених рядків, зелена для доданих і рамка для видимої частини; клацніть або перетягніть, щоб прокрутити туди. Потрібна повна нова версія, тож порівняння кількох файлів чи знімок показують фрагменти, як і файл, завеликий для завантаження.

![Роздільний діф із підсвічуванням на рівні слів](../../screenshots/split-diff.webp)

Кнопка **{ }** на початку панелі інструментів відкриває [семантичний
підсумок](semantic-diff.md) — що змінилося, символ за символом, а не рядок за
рядком. Число на ній — скільки оголошень змінилося.
