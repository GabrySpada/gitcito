...
|---|---|
| **↑ / ↓** | 跳到上一处或下一处变更 |
| **完整文件**(仅分栏) | 整个文件，每处变更都在原位；关闭时只显示变更的区块 |
...
**完整文件。** 分栏视图显示整个文件——左边是旧版本，右边是新版本——并在第一处变更处打开：红色和绿色色带标出变更的行，没有对应行的一侧用斜线填充，缩进参考线标出每一级缩进，差异右侧的概览标尺显示整个文件——红色通道表示删除的行，绿色通道表示新增的行，方框表示当前可见的部分；点击或拖动即可滚动过去。这需要完整的新版本，所以多文件比较或快照显示的是区块，太大而无法加载的文件也一样。

![带词级高亮的分栏差异](../../screenshots/split-diff.webp)

工具栏开头的 **{ }** 按钮会打开[语义摘要](semantic-diff.md)——按符号而不是按行显示改了什么。按钮上的数字是改动的声明数量。
...
工作树文件的**文件视图**——无论是否已暂存、是否被跟踪、还是全新文件——都会在自 HEAD（暂存文件则为索引）以来发生变化的每一行旁显示一条彩色条：绿色表示新增，蓝色表示修改，未被替换而直接删除的地方会出现一个小红色楔形。点击条形即可在不离开文件的情况下查看该更改——弹出框会显示被删除和新增的行，并提供上一个/下一个按钮，用于逐一查看文件中的所有更改。

![更改标尺与打开的更改弹出框，右侧为缩略图](../../screenshots/change-gutter.webp)

**缩略图**默认关闭，会在右侧边缘添加整个文件的缩放概览——点击或拖动即可跳转，就像滚动条要是还能显示文件形状一样。这两项都是按机器设置的开关，位于**设置 → 通用 → 文件查看器**。

两者都不会读取历史记录：切换到 Diff、Blame 或更早的提交时标尺就会消失——因为那些视图本来就直接显示每一处更改。
